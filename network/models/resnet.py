...
class ResNet(nn.Module):
    def __init__(self, builder, block, layers, num_classes=1000, base_width=64):
        ...
        if args.last_layer_dense:
            ...
            self.fc16 = builder.conv1x1(16, num_classes)
            self.fc32 = builder.conv1x1(32, num_classes)
            self.fc64 = builder.conv1x1(64, num_classes)
            self.fc128 = builder.conv1x1(128, num_classes)
            self.fc256 = builder.conv1x1(256, num_classes)
    ...
    def forward(self, x, get_rep=False):
        ...
        out = self.avgpool(x)
        out2048 = self.fc(out)
        out16 = self.fc16(out[:,0:16,:,:])
        out32 = self.fc32(out[:,0:32,:,:])
        out64 = self.fc64(out[:,0:64,:,:])
        out128 = self.fc128(out[:,0:128,:,:])
        out256 = self.fc256(out[:,0:256,:,:])
        # x = x.view(x.size(0), -1)
        if get_rep:
            return out2048.flatten(1), out16.flatten(1), out32.flatten(1), out64.flatten(1), out128.flatten(1), out256.flatten(1), out.flatten(1)
        else:
            out2048.flatten(1), out16.flatten(1), out32.flatten(1), out64.flatten(1), out128.flatten(1), out256.flatten(1)
